...
export function resolveAllScrimResults(
  ...
  if (Array.isArray(tournament.manualResults) && tournament.manualResults.length > 0) {
    tournament.manualResults.forEach((m: any, idx: number) => {
      ...
      competitorMap.set(key, {
        ...
        teamId: m.teamId,
        userId: m.userId,
        leader: m.leader || m.username || teamName,
  ...
  slotsSource.forEach((slot: any) => {
    ...
    competitorMap.set(key, {
      ...
      teamId: slot.teamId,
      userId: slot.userId,
      leader: slot.leader || slot.inGameName || teamName,
  ...
  if (Array.isArray(participants) && participants.length > 0) {
    participants.forEach((p: any) => {
      ...
      competitorMap.set(key, {
        ...
        teamId: p.teamId,
        userId: p.userId,
        leader: p.username || p.inGameName || teamName,
  ...
  winnersByKey.forEach((w: any) => {
    ...
    if (!competitorMap.has(key)) {
      competitorMap.set(key, {
        ...
        teamId: w.teamId,
        userId: w.userId,
        leader: w.leader || w.username || w.teamName,
